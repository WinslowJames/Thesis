...
# model
simple_model <- `colnames<-`(cast_thesis, c("id","time.to.death","n.events","dead"))
...
# Let's rescale hospital number also
Reshaped$hospital <- as.numeric(Reshaped$hospital)
table(sort(as.factor(Reshaped$hospital)))
Reshaped$hospital <- factor(Reshaped$hospital, ordered = is.ordered(x))
...
Reshaped$race <- recode(Reshaped$race, " 'White' = 'White' ; 'Black' = 'Black'; else = 'Other' ")
...
#              maxit=1e4
#             
	mod1 <- optim(rep(0,27), joint.llik.cov, hessian=TRUE, N=Reshaped$n.events, y=Reshaped$time.to.death, delta=Reshaped$dead, z=z,
...
# 	betacoef <- tail(regcoef,length(regcoef)/2)
# 
# 	lambda0 <- exp(cbind(1,z) %*% betacoef)
# 	mu0 <-  exp(cbind(1,z) %*% alphacoef)
```
